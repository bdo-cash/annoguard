...
/**
 * <i>Weak Keep</i> all non-static fields in the class.<br>
 * Scope: any types/classes.<br>
 * Basic rules see {@link Keep}.<br>
 * Note: there is no implementations of <i>Strong Keep</i> for all non-static fields,
 * in order to prevent unnecessary expand the scope of keep.
 * because:
 * <ul>
 * <li>for the public fields, it's really unnecessary.
 * <li>usually need kept only for public, can use {@link KeepVp$} instead.
 * </ul>
 * If still cannot meet the demand, can use {@link Keep$} aim at fields one by one.
 * <p>
 * <i>弱保留</i> 内部所有非static字段。<br>
 * 使用范围：任何类型。<br>
 * 基本规则参见 {@link Keep}.<br>
 * 注意：这里没有<i>强保留</i> 所有非static字段的实现，以防止不必要的扩大keep范围。因为：
 * <ul>
 * <li>对于非public字段，几乎是不应该keep的。
 * <li>通常需要keep的只是public，可用{@link KeepVp$}.
 * </ul>
